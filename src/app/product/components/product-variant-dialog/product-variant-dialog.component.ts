...
export class ProductVariantDialogComponent implements OnInit {
  ...
  get selectedVariantImage(){
    ...
    return this.variants[this.selectedVariantIndex].productVariantImages[0].imageUrl;
  }

  get selectedVarinantName(){
    if(this.selectedVariantIndex < 0 || this.variants.length === 0){
      return '';
    }
  ...
  handleSelection(indx){
    this.selectedVariantIndex = indx;
